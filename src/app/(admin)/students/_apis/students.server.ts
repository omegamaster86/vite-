...
import { GetStudentsResponseSchema } from "@/types/schemas";

/**
 * 学生一覧を取得（Edge Function経由）
...
export async function getStudents(params?: {
  ...
  try {
    ...
    if (authResult.errorCode === "unauthorized") {
      ...
      return { students: [], total: 0, page: 1, pageSize: 10 };
    }
    ...
    const yearQuery = Number.isFinite(year) ? `&year=${year?.toString()}` : "";
    const monthQuery = Number.isFinite(month)
      ? `&month=${month?.toString()}`
      : "";

    const response = await fetch(
      `${supabaseUrl}/functions/v1/get-students-for-admin?page=${page.toString()}${yearQuery}${monthQuery}`,
      {
        method: "GET",
        headers: {
    ...
    const parsed = GetStudentsResponseSchema.safeParse(json);
    if (!parsed.success) {
    ...
    const students: StudentInfo[] = parsed.data.data.map((row) => {
      return {
        id: String(row.id),
        name: row.name,
