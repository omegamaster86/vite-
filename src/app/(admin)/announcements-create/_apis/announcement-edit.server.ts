...
import type { AnnouncementEditDetail } from "@/types";
import { GetAnnouncementEditDetailResponseSchema } from "@/types/schemas";

/**
 * お知らせ編集画面の初期値を取得（Edge Function経由）
 * - 認証検証は getUser() を使用（必須）
 * - access_token が必要なため getSession() で取得
 */
export async function getAnnouncementEditDetail(
  announcementId: number,
): Promise<AnnouncementEditDetail | null> {
  cookies();

  const logger = createLogger("getAnnouncementEditDetail");
  logger.start({ announcementId });
  ...
  try {
    ...
    const response = await fetch(
      `${supabaseUrl}/functions/v1/get-announcement-edit-detail-for-admin?announcementId=${encodeURIComponent(String(announcementId))}`,
      {
        method: "GET",
        headers: {
    ...
    const parsed = GetAnnouncementEditDetailResponseSchema.safeParse(json);
    if (!parsed.success) {
    ...
    const detail = parsed.data.data satisfies AnnouncementEditDetail | null;
